...
class userController {
    ...
    async changePassword(req, res) {
        try {
            ...
            if (!user) {
                return res
                    .status(400)
                    .json({ msg: "Bạn nên đăng nhập lại nếu có tài khoản." });
            }
    ...
    async adminGetOne(req, res) {
        try {
            const user = await User.findById(req.params.id)
                .select("-password")
                .populate("cart.user_cart");
            const newUser = await User.findById(req.user.id);
            if (!user) {
                return res
                    .status(400)
                    .json({ msg: "Tài khoản không tồn tại." });
            }
            if (
                ...
                return res
                    .status(400)
                    .json({ msg: "Bạn không thể làm việc này." });
            }
    ...
    async addCart(req, res) {
        try {
            ...
            const check = user.cart.every(
                (item) =>
                    item.user_cart.toString() !== cart.user_cart.toString()
            );
            if (!check) {
                return res
                    .status(400)
                    .json({ msg: "Bạn đã có sản phẩm này trong rỏ hàng." });
            }
            ...
            res.status(200).json({
                msg: "Thêm thành công sản phẩm vào rỏ hàng của bạn.",
            });
        } catch (err) {
            return res.status(500).json({ msg: err.message });
        }
    ...
    async updateUser(req, res) {
        try {
            ...
            if (!user) {
                return res
                    .status(400)
                    .json({ msg: "Tài khoản này không hề tồn tại." });
            }
            await User.findByIdAndUpdate(req.user.id, {
                image,
                name,
    ...
    async replaceCart(req, res) {
        try {
            ...
            user.cart = user.cart.filter(
                (item) => item.user_cart.toString() !== req.params.id.toString()
            );
            await User.findByIdAndUpdate(req.user.id, {
                cart: user.cart,
            });
    ...
    async addCount(req, res) {
        try {
            ...
            user.cart = user.cart.map((item) => {
                if (
                    item.user_cart._id.toString() === req.params.id.toString()
                ) {
                    item.count = count;
                }
                return item;
